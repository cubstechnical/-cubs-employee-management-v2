...
async function globalSetup(config: FullConfig) {
  ...
  try {
    ...
    const passwordInput = page.getByPlaceholder('Enter your password').or(page.getByPlaceholder('Password')).or(page.locator('input[type="password"]'));
    
    await emailInput.fill('info@cubstechnical.com');
    await passwordInput.fill('Admin@123456');
    console.log('🔑 Credentials filled');
    
    // Click sign in button - try multiple button variations
    const signInButton = page.getByRole('button', { name: 'Sign In' }).or(page.getByRole('button', { name: 'Login' })).or(page.locator('button[type="submit"]'));
    ...
    const errorMessages = page.locator('[role="alert"], .error, .text-red-600, .text-red-500, .alert-error');
    ...
    if (currentUrl.includes('/dashboard') || currentUrl.includes('/admin') || currentUrl.includes('/')) {
